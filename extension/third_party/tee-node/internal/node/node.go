...
// SigningKeyEnvVar, when set, supplies a fixed node identity private key
// (32-byte secp256k1 scalar, hex, "0x" optional) so the derived teeID is
// stable across process restarts and container recreation. When unset,
// Initialize generates a fresh key on every start (unchanged behavior).
const SigningKeyEnvVar = "TEE_SIGNING_KEY"

// Initialize sets the node's private key and derives the teeID. If
// SigningKeyEnvVar is set the node adopts that fixed identity; otherwise
// it generates a fresh key (legacy behavior).
func Initialize(state State) (*Node, error) {
	privKey, err := signingKeyFromEnvOrGenerate()
	if err != nil {
		return nil, err
	}
...
// signingKeyFromEnvOrGenerate returns the node identity key. If
// SigningKeyEnvVar is set, its value must be a 32-byte secp256k1 scalar in
// hex ("0x" optional); the same value yields the same teeID across
// restarts. If unset, a fresh key is generated.
func signingKeyFromEnvOrGenerate() (*ecdsa.PrivateKey, error) {
	keyB, isSet, err := bytesFromEnv(SigningKeyEnvVar)
	if !isSet {
		return crypto.GenerateKey()
	}
	if err != nil {
		return nil, fmt.Errorf("invalid %s: %w", SigningKeyEnvVar, err)
	}
	if len(keyB) != 32 {
		return nil, fmt.Errorf("invalid %s: got %d bytes, want 32", SigningKeyEnvVar, len(keyB))
	}
	privKey, err := crypto.ToECDSA(keyB)
	if err != nil {
		return nil, fmt.Errorf("invalid %s: %w", SigningKeyEnvVar, err)
	}
	return privKey, nil
